...
static int find_mmap_areas(uint64 start,int len)
{
    ...
    for(int i =0;i<MMAP_NUM;i++)
    {
        //printk("finding: start=%p, len=%d\n",current->mmap_areas[i].start,current->mmap_areas[i].len);
...
uint64 do_mmap(void* start,size_t len,int port,int flags,int fd,off_t offset)
{
    ...
    if(fd < 0 || fd>=N_OPEN_FILE || offset <0 || len <0 || (uint64)start < 0)
        return err; // 异常情况

    if(start != NULL && current->mmap_va_available+len > MMAP_END_VA)
    ...
    if(port& PROT_READ) 
        permission  |= (PTE_R | PTE_A);//读权限
    if(port& PROT_WRITE)
        permission  |= (PTE_W | PTE_D);//写权限
    if(port&PROT_EXEC)
        permission  |= (PTE_X | PTE_A);//执行权限
      
    uint64 start_mmap_addr = current->mmap_va_available; // 开始映射的虚拟地址，注意写回  
    ...
    //段记录,递增记录
    current->segment_map_info[current->segment_num].va = current->mmap_va_available;
...
int do_munmap(void* start,size_t len)
{
    ...
    uint64 start_mmap_addr = seg_map_info->va;// 开始映射的虚拟地址
    ...
    for(int i =0;i<page_num;i++)
    {
        void* pa = va_to_pa(current->pagetable,(void*)start_mmap_addr);// 将虚拟地址转化为物理地址
        if(is_writeback)
        {   // 写回原文件
